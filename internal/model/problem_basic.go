...
type ProblemBasic struct {
	...
	Difficulty        int                `gorm:"column:difficulty;type:tinyint;" json:"difficulty"`
	DifficultyMode    int                `gorm:"column:difficulty_mode;type:tinyint;" json:"difficulty_mode"`
	PassRate          int                `gorm:"column:pass_rate;type:int(11);" json:"pass_rate"` // -1 = N/A
	TestCases         []*TestCase        `gorm:"foreignKey:problem_identity;references:identity" json:"test_cases"`
	ProblemTags       []*ProblemTag      `gorm:"foreignKey:problem_identity;references:identity" json:"problem_tags,omitempty"`
}
...
func GetProblemList(keyword string, categoryIdentity string, difficulty int) *gorm.DB {
	// 转义 LIKE 通配符防止注入
	escaped := keyword

...
func GetUserBasic(c *gin.Context) {
	...
	err := model.GetUserList(identity).Omit("Password").Find(&data).Error
	if err != nil {
...
func Login(c *gin.Context) {
	...
	// 先按用户名查询用户
	data := new(model.UserBasic)
	err := model.DB.Where("name = ?", username).First(data).Error
	if err != nil {
		if err == gorm.ErrRecordNotFound {
			c.JSON(200, gin.H{
				"code": -1,
	...
	if !helper.CheckPassword(password, data.Password) {
		...
		model.DB.Model(data).Update("password", newHash)
	}
...
func SendCode(c *gin.Context) {
	...
	// 邮箱格式校验
	if mail == "" {
		c.JSON(200, gin.H{"code": -1, "msg": "邮箱不能为空"})
		return
	}
	if !helper.ValidateEmail(mail) {
		c.JSON(200, gin.H{"code": -1, "msg": "邮箱格式不正确"})
		return
	}

	// 发送频率限制：60 秒内同邮箱不可重复发送
	cooldownKey := "sendcode:cooldown:" + mail
	exists, _ := model.RDB.Exists(config.CTX, cooldownKey).Result()
	if exists > 0 {
		ttl, _ := model.RDB.TTL(config.CTX, cooldownKey).Result()
		c.JSON(200, gin.H{
			"code": -1,
			"msg":  fmt.Sprintf("发送过于频繁，请 %d 秒后再试", int(ttl.Seconds())),
		})
		return
	}
	...
	// 先存 Redis 再发邮件，避免邮件发出但验证码没存的情况
	model.RDB.Set(config.CTX, mail, code, time.Second*300)
	model.RDB.Set(config.CTX, cooldownKey, "1", time.Second*60)

	err := helper.SendEmail(mail, code)
	if err != nil {
		log.Printf("发送邮件失败: %v", err)
		// 邮件失败时删除验证码，防止无效 code 残留
		model.RDB.Del(config.CTX, mail)
		model.RDB.Del(config.CTX, cooldownKey)
		c.JSON(200, gin.H{"code": -1, "msg": "邮件发送失败，请检查邮箱地址或稍后重试"})
		return
	}
	c.JSON(200, gin.H{"code": 200, "msg": "验证码已发送"})
}
...
func Register(c *gin.Context) {
	...
	//验证码是否正确
	GetCode, err := model.RDB.Get(config.CTX, mail).Result()
	if err != nil {
	...
	//判断邮箱是否存在
	var count int64
	err = model.EmailExist(mail).Count(&count).Error
	if err != nil {
		c.JSON(200, gin.H{
			"code": -1,
			"msg":  "服务器异常，请稍后再试",
	...
	err = model.InsertUser(user).Error
	if err != nil {
		c.JSON(200, gin.H{
			"code": -1,
			"msg":  "用户创建失败",
...
func GetRankList(c *gin.Context) {
	...
	// 先 Count 再分页查询
	err = model.GetRankList().Count(&count).Offset(page).Limit(size).Find(&list).Error
	if err != nil {
		c.JSON(200, gin.H{
			"code": -1,
			"msg":  "获取数据失败",
